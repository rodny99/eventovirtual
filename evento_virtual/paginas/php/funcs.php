...
require 'conexion.php';
	...
	function isNull($nombres, $user, $pass, $pass_con, $email){
		if(strlen(trim($nombres)) < 1 || strlen(trim($user)) < 1 || strlen(trim($pass)) < 1 || strlen(trim($pass_con)) < 1 || strlen(trim($email)) < 1)
		{
			return true;
			} else {
			return false;
	...
	function usuarioExiste($usuario)
	{
		...
		$stmt = mysqli_prepare($conexion,"SELECT id FROM usuarios WHERE usuario = ? LIMIT 1");
	...
	function emailExiste($email)
	{
		...
		$stmt = mysqli_prepare($conexion,"SELECT id FROM usuarios WHERE correo = ? LIMIT 1");
	...
	function registraUsuario($tipoid,$identificacion,$nombres,$apellidos,$email,$tipo_usuario,$usuario,$pass_hash,$activo, $token){
		
		global $mysqli;
		
		$stmt = $mysqli->prepare("INSERT INTO usuarios (tipoid,identificacion,nombres,apellidos,correo,tipo_usuario,usuario, password,activacion,token) VALUES(?,?,?,?,?,?,?,?,?,?)");
		$stmt->bind_param('sssssissis', $tipoid,$identificacion,$nombres,$apellidos,$email,$tipo_usuario,$usuario,$pass_hash,$activo, $token);
